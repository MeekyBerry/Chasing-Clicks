...
const Clicks = () => {
  ...
  useEffect(() => {
    ...
    if (map) {
      map.on("click", async (e) => {
        ...
        const data = await response.json();
        const features = data.features;
        // Loop through the features to find the country
        for (let i = 0; i < features.length; i++) {
          const feature = features[i];
          if (feature.place_type.includes("country")) {
            setCountry(feature.text);
            localStorage.setItem("country", feature.text);
            break;
          }
        }
        // Loop through the features to find the state
        for (let i = 0; i < features.length; i++) {
          const feature = features[i];
          if (feature.place_type.includes("region")) {
            setState(feature.text);
            localStorage.setItem("state", feature.text);
            break;
          }
        }
        ...
        const clicksByLocation =
          JSON.parse(localStorage.getItem("clicksByLocation")) || {};
        const key = `${country}-${state}`;
        const count = clicksByLocation[key] || 0;
        clicksByLocation[key] = count + 1;
        localStorage.setItem(
          "clicksByLocation",
          JSON.stringify(clicksByLocation)
        );
  ...
  return (
    <div>
      ...
      <button onClick={handleClick}>Click Me</button>
      <p>
        You clicked me <strong>{count}</strong> times.
      </p>
      {country && state && (
        <p>
          You clicked me from {state}, {country}.
