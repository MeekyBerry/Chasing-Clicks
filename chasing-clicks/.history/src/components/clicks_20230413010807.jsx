  ...
  const handleButtonClick = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        if ( !Object.keys(clicksByLocation).includes(`${state}, ${country}`)
        ) {
          setClicksByLocation({
            ...clicksByLocation,
            [`${state}, ${country}`]: 1,
          ...
          setClicksByLocation({
            ...clicksByLocation,
            [`${state}, ${country}`]: clicksByLocation[`${state}, ${country}`] + 1,
          });
        }
        // const popup = new mapboxgl.Popup().setHTML(
        //   `${state}, ${country} has been clicked ${clicksByLocation[`${state}, ${country}`]} times`
        // );
    ...
    <div>
      ...
      <ul>
        {Object.keys(clicksByLocation).map((location) => (
          <li key={location}>
            {state}, {country} has been clicked {clicksByLocation[location]} times
          </li>
        ))}
      </ul>
