...
const ClickCounter = () => {
  ...
  return (
    <div className="click">
      <h1 className="click--title">Chasing the clicks</h1>
      {/* Conditionally render count */}
      {count > 0 && (
        <p className="click--text">
          I have been clicked <strong className="click--text__count">{count}</strong> times.
        </p>
      )}
      {/* Conditionally render state and country */}
      {state && country && (
        <p className="click--text">
          You clicked me from <strong className="click--text__location">{state},</strong>{" "}
      ...
      {Object.keys(clicksByLocation).length > 0 && (
        <div className="click--locationCount">
          <h2 className="click--locationCount__head">Clicks by Location</h2>
          <ul className="click--locationCount__list">
            {Object.keys(clicksByLocation).map((location) => (
              <li key={location} className="click--locationCount__list__item">
                <table className="click--locationCount__list__item--table">
                  <thead>
                    <tr className="click--locationCount__list__item--table__head">
                      <th>Locations</th>
                      <th>Clicks</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr className="click--locationCount__list__item--table__body">
                      <td>{location}</td>
                      <td>{clicksByLocation[location]}</td>
                    </tr>
                  </tbody>
                </table>
                {/* <p>{location} {clicksByLocation[location]}</p> */}
              </li>
            ))}
          </ul>
        </div>
