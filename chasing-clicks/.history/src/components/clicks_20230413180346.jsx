...
const ClickCounter = () => {
  ...
  useEffect(() => {
    ...
    setCount(Object.values(clicksByLocation).reduce((acc, val) => acc + val, 0));
  }, [clicksByLocation]);
    ...
    <div className="click">
      ...
      {count > 0 && (
        <p className="click--text">
          I have been clicked{" "}
          <strong className="click--text__count">{count}</strong> times in total.
        </p>
      )}
      ...
      {state && country && (
        <p className="click--text">
          ...
          <strong className="click--text__location">{country}</strong><span className="click--text__location__mark">!</span>
        </p>
      )}
