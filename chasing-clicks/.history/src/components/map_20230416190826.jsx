...
const ClickCounterMap = () => {
  ...
  useEffect(() => {
    ...
    clickedLocations.forEach((click) => {
      ...
      newLocationCounts[locationKey] = (newLocationCounts[locationKey] || 0) + 1;
    });
    // Decrease the count of locations that are removed
    Object.keys(locationCounts).forEach((location) => {
      if (!(location in newLocationCounts)) {
        if (locationCounts[location] > 0) {
          newLocationCounts[location] = locationCounts[location] - 1;
        }
      } else {
        newLocationCounts[location] = Math.max(locationCounts[location], newLocationCounts[location]);
      }
    });
    // Update the state with the new location counts
    setLocationCounts(newLocationCounts);
  }, [clickedLocations, locationCounts]);
