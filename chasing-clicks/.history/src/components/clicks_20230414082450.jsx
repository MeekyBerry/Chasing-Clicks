  ...
  useEffect(() => {
    ...
    db.collection('clicks')
      ...
      .then(querySnapshot => {
        ...
        if (map) {
          const [mostRecentLocation] = Object.entries(clicksByLocation).sort((a, b) => b[1] - a[1]);
          if (mostRecentLocation) {
            const [lng, lat] = mostRecentLocation[0].split(',');
            map.flyTo({ center: [Number(lng), Number(lat)], zoom: 12 });
          }
        }
      })
      .catch(error => {
