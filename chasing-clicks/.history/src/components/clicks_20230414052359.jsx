...
const ClickCounter = () => {
  ...
  useEffect(() => {
    const savedCount = const savedCount = JSON.parse(localStorage.getItem("count"));
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        if (location.length > 0) {
          ...
          if (locationName && locationName.properties) {
            const { place_name, region, country } = locationName.properties;
            const location = `${place_name}, ${region}, ${country}`;
            setState(region);
            setCountry(country);
